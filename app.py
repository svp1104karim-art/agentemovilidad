...
db.init_db()
db.seed_from_csv(os.path.join(os.path.dirname(__file__), "dataset.csv"))

# --- INICIALIZAR SESIÓN ---
if 'df' not in st.session_state:
    st.session_state.df = db.obtener_todos()

# --- CONSTANTES ---
MODEL_PATH = os.path.join(os.path.dirname(__file__), "model.pkl")
REFERENCE_DATE = pd.to_datetime('01/01/2018')

# --- MODELO CACHEADO ---
@st.cache_resource(ttl=3600)
def entrenar_modelo_cached(df_actual):
    modelo_path = MODEL_PATH
    if os.path.exists(modelo_path) and not st.session_state.get('forzar_entrenamiento', False):
        with open(modelo_path, 'rb') as f:
            return pickle.load(f)
    df_temp = df_actual.copy()
    df_temp['DIAS_DESDE_REFERENCIA'] = (df_temp['FECHA_INFRACCION'] - REFERENCE_DATE).dt.days
    ...
    model.fit(X, y)
    os.makedirs(os.path.dirname(modelo_path) or '.', exist_ok=True)
    with open(modelo_path, 'wb') as f:
        pickle.dump((model, le_clase, le_codigo), f)
    return model, le_clase, le_codigo

model, le_clase, le_codigo = entrenar_modelo_cached(st.session_state.df)
...
<style>
    ...
    .stApp { background: radial-gradient(circle at 10% 20%, rgba(15,23,42,0.95) 0%, rgba(30,41,59,0.95) 90%); color: #f8fafc; }
    .metric-card { background-color: #1e293b; border-radius: 12px; padding: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.15); border: 1px solid #334155; border-left: 5px solid #3b82f6; transition: transform 0.3s ease; margin-bottom: 20px; color: #f8fafc; }
    .metric-card:hover { transform: translateY(-5px); box-shadow: 0 10px 25px rgba(0,0,0,0.25); }
    .metric-title { font-size: 13px; color: #94a3b8; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; }
    .metric-value { font-size: 26px; color: #f8fafc; font-weight: 700; margin-top: 8px; }
    .ticket-box { background-color: #1e293b; border: 1px solid #334155; border-radius: 12px; padding: 20px; margin-bottom: 1.5rem; }
    .ticket-title { color: #3b82f6; font-size: 1.2rem; font-weight: 700; margin-bottom: 0.5rem; }
    .ticket-detail-item { font-size: 0.9rem; color: #94a3b8; margin-bottom: 0.25rem; }
    [data-testid="stSidebar"] { background-color: #0f172a; border-right: 1px solid #334155; }
    [data-testid="stSidebar"] * { color: #e2e8f0; }
    .main-title { background: linear-gradient(135deg, #3b82f6 0%, #10b981 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 700; font-size: 36px; margin-bottom: 10px; }
    .ai-diag-card { background: rgba(16,185,129,0.05); border: 1px solid rgba(16,185,129,0.2); border-radius: 8px; padding: 15px; height: 100%; }
    .ai-diag-title { font-size: 0.85rem; color: #10b981; font-weight: 600; margin-bottom: 0.5rem; display: flex; align-items: center; gap: 6px; }
    .ai-diag-score { font-size: 1.6rem; font-weight: 700; margin-bottom: 5px; }
    .ai-diag-desc { font-size: 0.8rem; color: #94a3b8; }
</style>
...
def mostrar_modal_impugnacion(ticket_id, organismo):
    ...
    argumento = st.text_area("Argumento Legal:", placeholder="Describa las inconsistencias detectadas por la IA o los fundamentos de hecho...", height=120)
...
def refrescar_datos():
    st.session_state.df = db.obtener_todos()
    st.session_state['forzar_entrenamiento'] = True
    entrenar_modelo_cached.clear()
...
with st.sidebar:
    ...
    # Token desde secrets o sidebar
    verifik_token_secret = st.secrets.get("verifik", {}).get("token", None) if hasattr(st, "secrets") else None
    if verifik_token_secret:
        st.session_state['verifik_token'] = verifik_token_secret
        st.success("✅ Token Verifik cargado desde secrets")
    else:
        st.markdown("### 🌐 API SIMIT en Vivo (Verifik)")
        verifik_token_input = st.text_input("Token de Verifik.co:", type="password", placeholder="Bearer token de tu cuenta", help="Obtén tu token gratuito en verifik.co")
        if verifik_token_input:
            st.session_state['verifik_token'] = verifik_token_input
    ...
    archivo_usuario = st.file_uploader("Sube tu archivo de comparendos reales:", type=["csv", "xlsx"], help="Sube un archivo Excel o CSV que tenga columnas similares a: ID_COMPARENDO, CC_USUARIO, PLACA, COD_INFRACCION, CLASE_VEHI, VALOR_SANCION, ESTADO, etc.")
    if archivo_usuario is not None:
        try:
            ...
            db.insertar_muchos(df_cargado)
            refrescar_datos()
            st.success("¡Datos reales cargados y guardados en BD con éxito!")
        except Exception as e:
            st.error(f"Error al procesar el archivo: {e}")

    st.write("---")
    opcion = st.radio("Navegación", ["📊 Dashboard de Control", "🔍 Diagnóstico de Comparendos", "⚖️ Guía Legal y Plantillas"])
    st.write("---")
    st.info("💡 **Versión:** 2.0.0 (Producción)\n\nPersistencia SQLite + Modelo cacheado.")
...
if opcion == "📊 Dashboard de Control":
    ...
    st.caption(f"📦 {len(st.session_state.df)} registros en BD • {db.get_conn().execute('SELECT COUNT(*) FROM comparendos').fetchone()[0]} persistentes")
    ...
    with col1:
        st.markdown(f"""<div class="metric-card" style="border-left-color: #3b82f6;"><div class="metric-title">Comparendos Totales</div><div class="metric-value">{total_comparendos}</div></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown(f"""<div class="metric-card" style="border-left-color: #ef4444;"><div class="metric-title">Con Inconsistencias</div><div class="metric-value">{num_inconsistencias} <span style="font-size:14px; color:#ef4444;">({tasa_irregularidad:.1f}%)</span></div></div>""", unsafe_allow_html=True)
    with col3:
        st.markdown(f"""<div class="metric-card" style="border-left-color: #10b981;"><div class="metric-title">Recaudo Total Registrado</div><div class="metric-value">${total_pesos:,.0f}</div></div>""", unsafe_allow_html=True)
    with col4:
        st.markdown(f"""<div class="metric-card" style="border-left-color: #f59e0b;"><div class="metric-title">Ahorro Impugnable</div><div class="metric-value">${total_ahorro_potencial:,.0f}</div></div>""", unsafe_allow_html=True)
...
# --- PÁGINA 2: DIAGNÓSTICO ---
    ...
    with tabs[0]:
        ...
        st.markdown("""<div style="background-color: #1e293b; padding: 20px; border-radius: 12px; border: 1px solid #334155; margin-bottom: 25px;"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;"><h4 style="margin: 0; color: #f8fafc;">Consultar Base de Datos Local</h4><span style="font-size: 0.85rem; color: #94a3b8;">🔒 Búsqueda Segura</span></div>""", unsafe_allow_html=True)
        ...
        if resultados is not None:
            if len(resultados) == 0:
                ...
                for index, row in resultados.iterrows():
                    ...
                    dias = (pd.to_datetime(fecha) - REFERENCE_DATE).days
                    try:
                    ...
                    with col_diag:
                        ...
                        st.markdown(f"""<div class="ai-diag-card"><div class="ai-diag-title">⚖️ Diagnóstico IA</div><div class="ai-diag-score" style="color: {score_color};">{viabilidad_score}% Viabilidad</div><div class="ai-diag-desc"><strong>Predicción:</strong> {prediccion}<br><em>Motivo: {inconsistencia_just}</em></div></div>""", unsafe_allow_html=True)
                    with col_actions:
                        st.write("")
                        st.write("")
    ...
    with tabs[2]:
        ...
        with st.form("registro_comparendo_real"):
            ...
            inconsistencia_manual = st.selectbox("¿Tiene inconsistencias conocidas? (Opcional)", [
                "SIN INCONSISTENCIAS",
            ...
            if enviar_registro:
                if not nuevo_id or not nueva_placa or not nuevo_organismo:
                    ...
                    nueva_fila = {
                        ...
                        'FECHA_INFRACCION': nueva_fecha.strftime('%Y-%m-%d'),
                    ...
                    db.insertar_registro(nueva_fila)
                    refrescar_datos()
                    st.success(f"🎉 Comparendo **{nuevo_id}** registrado exitosamente en la base de datos.")
                    st.balloons()
                    st.rerun()
...
# --- PÁGINA 3: GUÍA LEGAL ---
    ...
    with tab_doc2:
        ...
        """)
        st.code("""
# En tu .streamlit/secrets.toml (producción):
[verifik]
